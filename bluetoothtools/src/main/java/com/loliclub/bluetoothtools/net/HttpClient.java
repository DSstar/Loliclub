			...
			public void run() {
				try {
					...
					mHandler.post(new Runnable() {
						...
						public void run() {
							try {
								responseHandler.onSuccess(responseCode, new JSONObject(buffer.toString()));
							} catch (JSONException e) {
								e.printStackTrace();
//								responseHandler.onFailure(e, "JSONException");
								sendFailureMessage(responseHandler, e, "JSONException");
							}
						}
					});
...
//					responseHandler.onFailure(e, "MalformedURLException");
					sendFailureMessage(responseHandler, e, "MalformedURLException");
				} catch (SocketTimeoutException e) {
					e.printStackTrace();
//					responseHandler.onFailure(e, "SocketTimeoutException");
					sendFailureMessage(responseHandler, e, "SocketTimeoutException");
				} catch (IOException e) {
					e.printStackTrace();
//					responseHandler.onFailure(e, "IOException");
					sendFailureMessage(responseHandler, e, "IOException");
				}
			}
		});
	}
...
//					responseHandler.onFailure(e, "IOException");
	...
	protected static void sendFailureMessage(final HttpClientResponseHandler responseHandler, final Exception e, final String content) {
		mHandler.post(new Runnable() {
			@Override
			public void run() {
				responseHandler.onFailure(e, content);
			}
		});
	}
